...
export const Splash: FC<{}> = ({ setIsReady }) => {
  ...
  const animatedStyles = useAnimatedStyle(() => ({
    transform: [{ scale: loaded.value ? withSpring(10) : 1 }, { translateY: loaded.value ? withTiming(-40) : 0 }],
  ...
  return (
    <UIView reanimated center style={[styles.container, animatedStyles]}>
      <LottieView ref={animationRef} source={source} speed={1} loop={false} onAnimationFinish={() => (loaded.value = true)} style={styles.lottie} />
    </UIView>
  );
};
